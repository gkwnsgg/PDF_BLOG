...
def _build_text_from_stream_words(words: List[Dict[str, Any]],
    ...
    for key in sorted(lines.keys()):
        ...
        if pos_gaps:
            ...
            small_ref = 0.0
        thr = max(6.0, small_ref * float(inline_gap_mult), med_w * 1.25)
...
def extract_article_texts(
    ...
    for p in range(start_idx, stop_idx):
        ...
        def build_side_text(raw_spans: List[Dict[str,Any]], side_tag: str) -> Tuple[str, bool]:
            ...
            # 5) 제목 크기 대역 제거 + wm token 제거
                        # 5) 제목 크기 대역/워터마크/페이지넘버 토큰 제거 (단어 기준)
            ...
            for w in words_stream:
                ...
                if not txt:
                    continue
                # 워터마크 띠(top/bottom) 안에 있는 1~2글자짜리 막대기 토큰(l, I, |)은 본문에서 제거
                # 아래에서 in_wm_band를 계산하므로 여기서는 y대신 bbox로 직접 판정
                wyc_tmp = (w["bbox"][1] + w["bbox"][3]) / 2.0
                in_wm_band_tmp = (wyc_tmp <= top_band_y) or (wyc_tmp >= bot_band_y)
                if in_wm_band_tmp and len(txt) <= 2:
                    # Normalize: │→|, ｜→|, l→|, strip, remove trailing .-·•
                    norm_bar = txt.replace("│", "|").replace("｜", "|").replace("l", "|").strip()
                    norm_bar2 = re.sub(r"[.\-·•]+$", "", norm_bar)
                    norm_set = {"|", "l", "I"}
                    # Case-insensitive check for "l", "I", "|"
                    norm_cmp = norm_bar2.strip().upper()
                    if norm_cmp in {"|", "L", "I"}:
                        continue
                # 제목 크기 대역은 본문에서 제외
                if low <= sz <= high:
...
# 사용자가 --no-stream-order 안 줬으면 스트림 사용
    ...
    #지오메트리/장식문구 스킵/캡션프루닝/서브타이틀 밴드 비활성화 플래그
    ...
    setattr(args, "_disable_subtitle_band", True)
